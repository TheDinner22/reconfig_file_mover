...
int main(int argc, char *argv[]) {
  ...
  for (unsigned i = 0; i < TEST_SIZE; i++) {
    ...
    // bool Board::write(unsigned int *data, unsigned long addr, unsigned long
    // words);
    //
    // Parameters:
    // -------------------------------------
    ...
    // bool Board::read(unsigned int *data, unsigned long addr, unsigned long
    // words);
    //
    // Parameters:
    // -------------------------------------
    ...
    if (correctOut != out[i]) {
      cerr << "Error: Board output for " << in0[i] << "*" << in1[i] << " is " << out[i] << " instead of "
           << correctOut << endl;
      return -1;
    }
  }
